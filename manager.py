...
def price_format(price, length=3):
    price = str(price)
    res = []
    counter = -length
...
class DataManager:
    ...
    def write_columns(self, columns, sizes):
        ...
        for index, value in enumerate(columns, start=1):
            ...
            if index == 5:
                self.write_cell(1, index, value, fill=year_fill, align=align)
            else:
    ...
    def write_data(self, data):
        ...
        for index, auto in enumerate(data, start=2):
            ...
            self.write_cell(index, 6, price_format(auto['price']), align=align)
            self.write_cell(index, 7, auto['discount'], align=align)
            self.write_cell(index, 8, price_format(auto['final_price']), align=align)
        ...
        for pwt, models in pwts.items():
            for model, price in models.items():
                ...
                self.write_cell(amount + index, 8, price_format(price), align=align)
                index += 1
            index += 1